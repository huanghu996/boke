...
//	cout<<num(1,4);
	 ...
	 for(int i=2;i<=n;i++)
	{
		for(int j=1;j<=m;j++)
		{
			...
			{
			;
				f[i][j]=maxt(f[i][j],f[k][j-1]*num(k+1,i));
				//cout<<"i="<<i<<" j= "<<j<<" k="<<k<<" f="<<f[i][j]<<" "<<"f["<<k<<"]["<<j-1<<"]="<<f[k][j-1]*num(k+1,i)<<" ";
			}
		}
		//cout<<endl;
	} 
...
```
#### 类区间动归
i表示状态（从1到i个的情况），j表示“X”号的个数，k用来计算i中的最优值，f[]存积和（是题目问的）。

为何如此表示状态？

f[]存题目问的和与积，不需要用状态表示。因为题目的操作是插入乘号，如果用经典区间动归的话不仅不方便进行乘号个数的限制，而且根本算不出，因为经典动归需要先要把每两的和算出来，在算每3个的和，在算每4个……必定会影响后面的情况。
